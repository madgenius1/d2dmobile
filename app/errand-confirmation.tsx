...
import { useAppContext, PendingErrand, ErrandType } from "@/context/AppContext";

const TYPE_LABELS: Record<string, string> = {
  grocery: "Grocery Run", pharmacy: "Pharmacy", document: "Document Drop", other: "Custom",
};

export default function ErrandConfirmationScreen() {
  const colors = useColors();
  const { pendingErrand, setPendingErrand, clearPendingErrand, addErrand } = useAppContext();
  const insets = useSafeAreaInsets();
  const [editing, setEditing] = useState<"pickup" | "dropoff" | "schedule" | null>(null);

  const draft: PendingErrand = pendingErrand ?? {
    type: "grocery", pickupRoute: "", pickupLocation: "Whole Foods Market, 5th Ave",
    dropoffRoute: "", dropoffLocation: "244 Oak St, North District",
    deliverToMe: true, pickupTime: "", instructions: "",
  };
  ...
  function saveSection() {
    setPendingErrand({ ...draft, pickupRoute, pickupLocation, dropoffRoute, dropoffLocation, pickupTime, instructions });
    setEditing(null);
  }
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 140 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.totalSection}>
          ...
          <Text style={[styles.totalAmount, { color: colors.primary }]}>$12.00</Text>
          <View style={[styles.speedBadge, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.speedText, { color: colors.foreground }]}>{TYPE_LABELS[draft.type] ?? "Errand"}</Text>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={styles.sectionHeader}>
            <View style={styles.dotLabel}>
              ...
              <Text style={[styles.sectionTitle, { color: colors.primary }]}>PICKUP (STORE)</Text>
            </View>
            <Pressable
              onPress={() => setEditing(editing === "pickup" ? null : "pickup")}
              style={[styles.editBtn, { backgroundColor: editing === "pickup" ? colors.primary : colors.secondary, borderColor: "#C8E6C9" }]}
            >
              <Ionicons name={editing === "pickup" ? "checkmark" : "pencil"} size={13} color={editing === "pickup" ? "#fff" : colors.primary} />
              <Text style={[styles.editBtnText, { color: editing === "pickup" ? "#fff" : colors.primary }]}>{editing === "pickup" ? "Done" : "Edit"}</Text>
            </Pressable>
          </View>
          {editing === "pickup" ? (
            <View style={styles.editArea}>
              ...
              <View style={[styles.inputWrap, { borderColor: colors.primary, backgroundColor: "#f8fff0" }]}>
                ...
                <TextInput value={pickupLocation} onChangeText={setPickupLocation} placeholder="Store & address" placeholderTextColor={colors.mutedForeground} style={[styles.input, { color: colors.foreground }]} />
              </View>
          ...
          <View style={styles.connectorRow}>
            ...
            <Text style={[styles.connectorLabel, { color: colors.mutedForeground }]}>COURIER ROUTE</Text>
          </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.cardLabel, { color: colors.mutedForeground }]}>SCHEDULE & INSTRUCTIONS</Text>
            <Pressable
              onPress={() => setEditing(editing === "schedule" ? null : "schedule")}
              style={[styles.editBtn, { backgroundColor: editing === "schedule" ? colors.primary : colors.secondary, borderColor: "#C8E6C9" }]}
            >
              <Ionicons name={editing === "schedule" ? "checkmark" : "pencil"} size={13} color={editing === "schedule" ? "#fff" : colors.primary} />
              <Text style={[styles.editBtnText, { color: editing === "schedule" ? "#fff" : colors.primary }]}>{editing === "schedule" ? "Done" : "Edit"}</Text>
            </Pressable>
          </View>
          {editing === "schedule" ? (
            <View style={styles.editArea}>
              <View style={[styles.inputWrap, { borderColor: colors.primary, backgroundColor: "#f8fff0" }]}>
                ...
                <TextInput value={pickupTime} onChangeText={setPickupTime} placeholder="e.g. Today, 5:00 PM" placeholderTextColor={colors.mutedForeground} style={[styles.input, { color: colors.foreground }]} />
              </View>
              <TextInput value={instructions} onChangeText={setInstructions} placeholder="Instructions..." placeholderTextColor={colors.mutedForeground} multiline numberOfLines={3} style={[styles.textarea, { color: colors.foreground, borderColor: colors.primary, backgroundColor: "#f8fff0" }]} />
              <Pressable onPress={saveSection} style={[styles.saveSectionBtn, { backgroundColor: colors.primary }]}>
            ...
            <View style={{ gap: 6 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                ...
                <Text style={[styles.summaryAddr, { color: colors.foreground, fontSize: 14 }]}>{pickupTime || "ASAP"}</Text>
              </View>
              {!!instructions && <Text style={[styles.summaryRoute, { color: colors.foreground, backgroundColor: colors.background, padding: 10, borderRadius: 8 }]}>{instructions}</Text>}
            </View>
          )}
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
            ...
            <Text style={[styles.summaryAddr, { color: colors.foreground, fontSize: 14 }]}>•••• 4242</Text>
            <Pressable style={{ marginLeft: "auto" }}>
              <Text style={[styles.editBtnText, { color: colors.primary }]}>CHANGE</Text>
            </Pressable>
          </View>
        </View>
      </ScrollView>
      ...
      <View style={[styles.actionBar, { backgroundColor: colors.card, borderTopColor: colors.border, paddingBottom: bottomPad + 12 }]}>
        <Pressable onPress={handleConfirm} disabled={!!editing} style={({ pressed }) => [styles.confirmBtn, { backgroundColor: editing ? colors.muted : colors.primary, opacity: pressed ? 0.85 : 1 }]}>
          <Text style={[styles.confirmBtnText, { color: editing ? colors.mutedForeground : "#fff" }]}>Confirm & Schedule</Text>
        </Pressable>
        <Pressable onPress={() => router.back()} style={({ pressed }) => [styles.backBtn, { borderColor: colors.primary, opacity: pressed ? 0.7 : 1 }]}>
...
const styles = StyleSheet.create({
  ...
  textarea: { borderWidth: 1.5, borderRadius: 10, padding: 10, fontSize: 14, fontWeight: "500", textAlignVertical: "top", minHeight: 80 },
