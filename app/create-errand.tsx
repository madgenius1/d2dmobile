...
export default function CreateErrandScreen() {
  ...
  const [selectedType, setSelectedType] = useState(pendingErrand?.type ?? "grocery");
  const [pickupRoute, setPickupRoute] = useState(pendingErrand?.pickupRoute ?? "");
  const [pickupLocation, setPickupLocation] = useState(pendingErrand?.pickupLocation ?? "");
  const [dropoffRoute, setDropoffRoute] = useState(pendingErrand?.dropoffRoute ?? "");
  const [dropoffLocation, setDropoffLocation] = useState(pendingErrand?.dropoffLocation ?? "");
  const [deliverToMe, setDeliverToMe] = useState(pendingErrand?.deliverToMe ?? false);
  const [pickupTime, setPickupTime] = useState(pendingErrand?.pickupTime ?? "");
  const [instructions, setInstructions] = useState(pendingErrand?.instructions ?? "");
  const [errors, setErrors] = useState<Record<string, string>>({});

  function handleDeliverToMe(val: boolean) {
    setDeliverToMe(val);
    if (val) {
      setDropoffLocation("244 Oak St, North District");
      setDropoffRoute("");
  ...
  function validate() {
    ...
    if (!pickupLocation.trim()) e.pickupLocation = "Store / pickup location is required";
    if (!deliverToMe && !dropoffLocation.trim()) e.dropoffLocation = "Drop-off location is required";
    return e;
  }
  ...
  function handleReview() {
    ...
    setPendingErrand({
      type: selectedType, pickupRoute, pickupLocation,
      dropoffRoute, dropoffLocation, deliverToMe, pickupTime, instructions,
    });
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      >
        {/* Type */}
        <Text style={[styles.label, { color: colors.foreground }]}>ERRAND TYPE</Text>
          ...
          {TYPES.map((t) => (
            <Pressable
              ...
              style={[
                styles.typeBtn,
                {
                  backgroundColor: selectedType === t.id ? colors.secondary : colors.card,
                  borderColor: selectedType === t.id ? colors.primary : colors.border,
                },
              ]}
            >
              <Ionicons
                name={t.icon}
                size={18}
                color={selectedType === t.id ? colors.primary : colors.mutedForeground}
              />
              <Text style={[styles.typeBtnText, { color: selectedType === t.id ? colors.primary : colors.foreground }]}>
                {t.label}
              </Text>
            </Pressable>
          ))}
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.routeRow}>
            ...
            <View style={styles.inputs}>
              <Text style={[styles.sectionLabel, { color: colors.primary }]}>PICKUP (STORE)</Text>
              ...
              <View style={[styles.inputWrap, { borderColor: errors.dropoffLocation ? colors.destructive : colors.border, opacity: deliverToMe ? 0.4 : 1 }]}>
                <Ionicons name="git-branch-outline" size={16} color={colors.mutedForeground} />
                <TextInput
                  value={dropoffRoute}
              ...
              <View style={[styles.inputWrap, {
                borderColor: errors.dropoffLocation ? colors.destructive : deliverToMe ? colors.primary : colors.border,
          ...
          <Pressable onPress={() => handleDeliverToMe(!deliverToMe)} style={styles.toggleRow}>
            <View style={{ flex: 1 }}>
              <Text style={[styles.toggleTitle, { color: colors.foreground }]}>Deliver to my address</Text>
              <Text style={[styles.toggleSub, { color: colors.mutedForeground }]}>Auto-fill drop-off with saved address</Text>
            </View>
            <View style={[styles.toggle, { backgroundColor: deliverToMe ? colors.primary : colors.muted }]}>
              <View style={[styles.thumb, { transform: [{ translateX: deliverToMe ? 22 : 2 }] }]} />
            </View>
          </Pressable>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.label, { color: colors.foreground }]}>PICKUP TIME</Text>
          <View style={[styles.inputWrap, { borderColor: colors.border, marginBottom: 14 }]}>
          ...
          <Text style={[styles.label, { color: colors.foreground }]}>SPECIAL INSTRUCTIONS</Text>
          <TextInput
            value={instructions}
      ...
      <View style={[styles.actionBar, { backgroundColor: colors.card, borderTopColor: colors.border, paddingBottom: bottomPad + 12 }]}>
        <View>
          <Text style={[styles.costLabel, { color: colors.mutedForeground }]}>EST. FEE</Text>
          <Text style={[styles.cost, { color: colors.primary }]}>$12.00</Text>
        </View>
        <Pressable
...
const styles = StyleSheet.create({
  ...
  textarea: {
    borderWidth: 1.5, borderRadius: 12, padding: 12, fontSize: 14,
    fontWeight: "500", textAlignVertical: "top", minHeight: 90,
  },
  actionBar: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    paddingHorizontal: 20, paddingTop: 14, borderTopWidth: 1.5,
  },
